...
class RFDiscriminator(nn.Module):
    ...
    This is applied to all compressed tokens in parallel.
    """
    ...
    def forward(self, tokens: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            tokens: (batch_size, hidden_dim) - single tokens
                   Can be (B*N, 1024) for processing all N tokens at once
...
if __name__ == "__main__":
    ...
    print("Testing RF Discriminator (Fixed 1024->512->256->1)...")

    hidden_dim = 1024
    batch_size = 4
    num_tokens = 64  # 8x8 grid

    # Create discriminator
    discriminator = RFDiscriminator(hidden_dim=hidden_dim, dropout=0.1)
